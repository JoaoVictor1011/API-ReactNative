...
export const Saldos = ({ route, navigation }) => {
    ...
    const SomarSaldos = (...saldos) => {
        ...
        try {
            if (somados !== null) {
                return somados.reduce((acumulador, valor) =>
                    acumulador + valor)
            } else {
                return 0
            }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.lista}>
                {saldos.map((item) =>
                    <Saldo key={item.id} id={item.id} idCliente={route.params.id}
                        nome={route.params.nome} valor={item.valor} navigation={navigation}
                    />)}
            </View>
        </View>
    )
